...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 2, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        ...
        if hasattr(l, "loras_A"):
            for param in l.loras_A:
                param.data = param.data.half()

        if hasattr(l, "loras_B"):
            for param in l.loras_B:
                param.data = param.data.half()

        for name in ["text_projection", "proj", "mcq_proj", "rgb_vproj", "ir_tproj", "ir_vproj", "rgb_tproj", "rgb_vproj"]:
            if hasattr(l, name):
                attr = getattr(l, name)
                if attr is not None:
