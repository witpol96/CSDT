...
class EvaluatorIR():
    ...
    def _compute_embedding(self, model):
        ...
        for pid, caption in self.txt_loader:
            caption = caption.to(device)
            with torch.no_grad():
                text_feat, _ = model.encode_ir_text(caption)
                text_feat = text_feat[torch.arange(text_feat.shape[0]), caption.argmax(dim=-1)].float().cpu()
            qids.append(pid.view(-1)) # flatten 
            qfeats.append(text_feat)
        qids = torch.cat(qids, 0)
        qfeats = torch.cat(qfeats, 0)
        ...
        for pid, img in self.img_loader:
            ...
            with torch.no_grad():
                # img_feat = model.encode_image(img).cpu()
                img_feat, _ = model.encode_ir_image(img)
                img_feat1 = img_feat[:,0,:].float().cpu()
            gids.append(pid.view(-1)) # flatten 
    ...
    def _compute_embedding_tse(self, model):
        ...
        for pid, caption in self.txt_loader:
            caption = caption.to(device)
            with torch.no_grad():
                text_feat, atten_t = model.encode_ir_text(caption)
                text_feat = model.ir_texual_emb_layer(text_feat, caption, atten_t)
...
class EvaluatorRGB():
    ...
    def _compute_embedding(self, model):
        ...
        for pid, caption in self.txt_loader:
            caption = caption.to(device)
            with torch.no_grad():
                text_feat, _ = model.encode_rgb_text(caption)
                text_feat = text_feat[torch.arange(text_feat.shape[0]), caption.argmax(dim=-1)].float().cpu()
            qids.append(pid.view(-1)) # flatten 
            qfeats.append(text_feat)
        qids = torch.cat(qids, 0)
        ...
        for pid, img in self.img_loader:
            img = img.to(device)
            with torch.no_grad():
                img_feat, _ = model.encode_rgb_image(img)
                img_feat1 = img_feat[:,0,:].float().cpu()
            gids.append(pid.view(-1)) # flatten 
            gfeats.append(img_feat1)
        gids = torch.cat(gids, 0)
